...
import com.online.music.mic.model.level_detail_modal.SingleLevelParticipation;
...
public class CompationDetailActivity extends BaseActivity implements View.OnClickListener {
    ...
    private String CompationName, CompationId, compatitionLevelId, compatitionLevelName, compatitionLevelDetail,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        CompationName = getIntent().getStringExtra("CompationName");
        ...
        if (strType.equalsIgnoreCase("second")) {
            ...
            if (resultStatus.equalsIgnoreCase("win")) {
                ...
                if (participation.getParticipationStatus()) {
                    if (participation.getType().equalsIgnoreCase("free")) {
                        if (participation.getAdminStatus().equalsIgnoreCase("active")) {
                            ...
                            if (strType.equalsIgnoreCase("top")) {
                                txtMessage.setVisibility(View.GONE);
                            } else {
                                txtMessage.setVisibility(View.VISIBLE);
                            }
                            Alerts.show(mContext, "Status Pending from admin side...!!!");
                            btnRank.setVisibility(View.GONE);
                            btnApply.setVisibility(View.GONE);
                        }
                    }else {
                ...
                public void onResponseSuccess(Response<?> result) {
                    ...
                    if (!loginModal.getError()) {
                       ...
                       /* btnApply.setVisibility(View.GONE);
                        ...
                        btnApply.setVisibility(View.GONE);
                        btnRank.setVisibility(View.VISIBLE);
                        btnContentUpload.setVisibility(View.GONE);
                    }
                }
    ...
    public void showAllDialog() {
        ...
        dialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
        dialog.setCancelable(false);
        dialog.setContentView(R.layout.activity_allfile_dialog);
        ImageView dialogButton = (ImageView) dialog.findViewById(R.id.cancleBtn);
        dialogButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
        ...
        final RadioGroup rgFile = (RadioGroup) dialog.findViewById(R.id.rgFile);
        Button btnSelectFile = (Button) dialog.findViewById(R.id.btnSelectFile);
        btnSelectFile.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                int selectedId = rgFile.getCheckedRadioButtonId();
                radioAudioButton = (RadioButton) dialog.findViewById(selectedId);
                if (radioAudioButton.getText().equals("Audio")) {
                    showAudioDialog();
                } else if (radioAudioButton.getText().equals("Video")) {
                   showVideoDialog();
                } else {
                    Toast.makeText(mContext, "Select Option any one option", Toast.LENGTH_SHORT).show();
                }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btnContentUpload:
                ...
                AppPreference.setStringPreference(mContext, Constant.COMPANY_ID, CompationId);
                AppPreference.setStringPreference(mContext, Constant.LEVEL_ID, compatitionLevelId);
                AppPreference.setStringPreference(mContext , Constant.FILE_TYPE, compatitonLevelContentType);
                AppPreference.setStringPreference(mContext , Constant.COMPETITION_NAME, CompationName);
                AppPreference.setStringPreference(mContext , Constant.COMPETITION_NAME_LEVEL, compatitionLevelName);
